...
sce <- readRDS("~/short_term_project/GSE157827_sce_ann.rds")
...
umap_coords <- reducedDim(sce)
colnames(umap_coords) <- colnames(reducedDim(sce))
...
plot_data <- plot_data %>%
  filter(!is.na(CellType), !is.na(Condition), !is.na(Expression)) %>%
  mutate(Condition = factor(Condition, levels = c("NC", "AD")))
...
cell_type_order <- c("pericyte", "fibroblast", "endothelial cell", "central nervous system macrophage",
  "leukocyte", "vascular associated smooth muscle cell", "astrocyte", "neuron", 
...
for (cell_type_of_interest in cell_types_to_analyze) {
  ...
  # Ensure the condition is a factor and set the reference level to "NC" (Control)
  metadata[[condition_column]] <- factor(metadata[[condition_column]], levels = c("NC", "AD"))
  ...
  # H. Extract results. contrast: 'AD' vs base level 'NC'
  res <- results(dds, contrast = c(condition_column, "AD", "NC"), alpha = 0.05)
...
# Save the plot
# ggsave("combined_sirt_forest_plot.png", combined_forest, width = 14, height = 8, dpi = 300)
