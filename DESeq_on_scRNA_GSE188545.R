...
if("condition" %in% colnames(colData(sce))) {
  table(colData(sce)$celltype, colData(sce)$condition) # rows - cell types, columns - condition
}

# Sample info (sometimes stored as "sample", "donor", or "batch")
colnames(colData(sce))
unique(colData(sce)$sample) # 21 samples 
...
# no UMAP in this dataset -> make it by my own
...
library(scater) # for single-cell analysis (work with SingleCellExperiment format file)
...
colnames(umap_coords) <- colnames(reducedDim(sce, 'UMAP'))
...
# First, create a reference UMAP colored by cell type
celltype_umap <- ggplot(umap_df, aes(x = UMAP1, y = UMAP2)) +
...
condition_umap <- ggplot(umap_df, aes(x = UMAP1, y = UMAP2)) +
  geom_point(aes(color = condition), size = 0.1, alpha = 1) + # alpha - semi-transparent (0.6)
...
plot_data <- plot_data %>%
  filter(!is.na(CellType), !is.na(Condition), !is.na(Expression)) %>%
  mutate(Condition = factor(Condition, levels = c("HC", "AD")))
...
hist(expr_values[expr_values > 0], breaks = 50, 
     main = "SIRT1 expression distribution",
     xlab = "Expression level (logcounts)",
...
summary(expr_values) # key statistics 
hist(
  expr_values[expr_values > 0],
  breaks = 50,
  main = "SIRT2 expression distribution",
  xlab = "Expression level (logcounts)",
  ylab = "Number of cells",
  col = "lightblue", border = "black"
)
...
hist(
  expr_values[expr_values > 0],
  breaks = 50,
  main = "SIRT3 expression distribution", 
  xlab = "Expression level (logcounts)",
  ylab = "Number of cells",
  col = "lightblue", border = "black"
)
...
hist(
  expr_values[expr_values > 0],
  breaks = 50,
  main = "SIRT4 expression distribution", 
  xlab = "Expression level (logcounts)",
  ylab = "Number of cells",
  col = "lightblue", border = "black"
)
...
hist(
  expr_values[expr_values > 0],
  breaks = 50,
  main = "SIRT5 expression distribution", 
  xlab = "Expression level (logcounts)",
  ylab = "Number of cells",
  col = "lightblue", border = "black"
)
...
hist(
  expr_values[expr_values > 0],
  breaks = 50,
  main = "SIRT6 expression distribution", 
  xlab = "Expression level (logcounts)",
  ylab = "Number of cells",
  col = "lightblue", border = "black"
)
...
hist(
  expr_values[expr_values > 0],
  breaks = 50,
  main = "SIRT7 expression distribution", 
  xlab = "Expression level (logcounts)",
  ylab = "Number of cells",
  col = "lightblue", border = "black"
)
...
combined_umap <- wrap_plots(sirt_umap_plots, ncol = 3) +
  plot_annotation(title = "SIRT Gene Expression Across Cell Types (UMAP)")
...
for (cell_type in cell_types) {
  ...
  for (sirt_gene in sirt_genes) {
    if (sirt_gene %in% rownames(logcounts_matrix)) {
      ...
      # Calculate mean of cells with expression > 0
      expressing_cells <- expr_values[expr_values > 0]
...
heatmap_plot <- ggplot(expr_long, aes(x = SIRT, y = reorder(cell_type, expression), fill = expression)) +
  ...
  labs(title = "Average SIRT expression across cell types (expressing cells only",
       x = "SIRT genes",
       y = "Cell types") +
  theme_minimal() +
  theme(
...
cell_type_order <- c("vascular associated smooth muscle cell", "leukocyte", 
                     "pericyte", "fibroblast", "endothelial cell", 
                     "central nervous system macrophage", "astrocyte", "neuron", 
...
for (cell_type_of_interest in cell_types_to_analyze) {
  ...
  # Ensure the condition is a factor and set the reference level to "HC" (Control)
  metadata[[condition_column]] <- factor(metadata[[condition_column]], levels = c("HC", "AD"))
  ...
  # H. Extract results. contrast: 'AD' vs base level 'HC'
  res <- results(dds, contrast = c(condition_column, "AD", "HC"), alpha = 0.05)
...
CenNervSysMacr_results <- deseq_results_list[["central nervous system macrophage"]]
...
# Create significance threshold column with padj <= 0.1
CenNervSysMacr_df$significant <- CenNervSysMacr_df$padj <= 0.1

# Create the volcano plot
volcano_plot <- ggplot(CenNervSysMacr_df, aes(x = log2FoldChange, y = -log10(pvalue))) +
  # Plot all points
  geom_point(aes(color = gene_type), size = 1.5, alpha = 0.6) +
...
sirt_forest <- ggplot(sirt_results, aes(x = log2FoldChange, y = reorder(gene, log2FoldChange))) +
  ...
  # Labels and theme
  labs(title = "SIRT gene expression changes in Central Nervous System Macrophages",
       subtitle = "AD vs Control with 95% confidence intervals",
       x = "Log2 Fold Change with 95% CI",
       y = "SIRT Genes") +
...
# Create significance threshold column with padj <= 0.05
neuron_df$significant <- neuron_df$FDR <= 0.05
...
volcano_plot <- ggplot(neuron_df, aes(x = logFC, y = -log10(PValue))) +
  ...
  # Highlight significant points (padj <= 0.05)
  geom_point(data = subset(neuron_df, significant == TRUE), 
             color = "red", size = 2.5) +
  
  # Add gene labels for significant points (padj <= 0.05)
  geom_text_repel(data = subset(neuron_df, significant == TRUE),
                  aes(label = gene),
                  color = "red",
                  size = 3,
  ...
  # Labels and title
  labs(title = "Differential Expression in Neurons: AD vs Control",
       subtitle = "Blue points: SIRT genes | Red points: padj ≤ 0.05",
       x = "log2 Fold Change (AD vs Control)",
       y = "-log10(p-value)") +
...
# Count significant DEGs (red dots)
significant_degs <- subset(neuron_df, significant == TRUE)
num_sig_degs <- nrow(significant_degs)

cat("=== Significant DEGs in Neurons (AD vs Control) ===\n")
cat("Total significant DEGs (FDR ≤ 0.05):", num_sig_degs, "\n")
...
# Create significance threshold column with padj <= 0.05
oligodendrocyte_df$significant <- oligodendrocyte_df$FDR <= 0.05
...
# Count significant DEGs (red dots)
significant_degs <- subset(oligodendrocyte_df, significant == TRUE)
num_sig_degs <- nrow(significant_degs)
...
# Create significance threshold column with padj <= 0.05
astrocyte_df$significant <- astrocyte_df$FDR <= 0.05
...
# Count significant DEGs (red dots)
significant_degs <- subset(astrocyte_df, significant == TRUE)
num_sig_degs <- nrow(significant_degs)
...
# Create significance threshold column with padj <= 0.05
oligodendrocytePrecursor_df$significant <- oligodendrocytePrecursor_df$FDR <= 0.05
...
# Count significant DEGs (red dots)
significant_degs <- subset(oligodendrocytePrecursor_df, significant == TRUE)
num_sig_degs <- nrow(significant_degs)
...
# Create significance threshold column with padj <= 0.05
CenNervSysMacr_df$significant <- CenNervSysMacr_df$FDR <= 0.05
...
# Count significant DEGs (red dots)
significant_degs <- subset(CenNervSysMacr_df, significant == TRUE)
num_sig_degs <- nrow(significant_degs)
